...
public class DragLayout extends ViewGroup {
	...
	public void computeScroll() {
		if (mDragHelper.continueSettling(true)) {
            ...
            if (frameView2.getTop() == 0) {
                //System.out.println("currentPage = 2;");
                currentPage = 2;
            } else if (frameView1.getTop() == 0) {
	...
	private void animTopOrBottom(View releasedChild, float yvel) {
		...
		if (releasedChild == frameView1) {
			...
			if (yvel > VEL_THRESHOLD
				...
				finalTop = viewHeight;
                if (null != nextPageListener) {
                    nextPageListener.onDragPrevious();
                }
			}
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		return shouldIntercept && yScroll && !mScrollView.isHorizontal;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mScrollView == null) {
            mScrollView = (CustScrollView) frameView1.findViewById(R.id.custScrollView);
        }
    ...
    public void setGoTopListener(OnGoTopListener goTopListener) {
